...
async function audit() {
    ...
    try {
        ...
        if (clientIssues > 0 || contactIssues > 0) {
            ...
            const clientRes = await db.collection('clients').updateMany({ organization: "" }, { $unset: { organization: 1 } });
            const contactRes = await db.collection('contacts').updateMany({ organization: "" }, { $unset: { organization: 1 } });
